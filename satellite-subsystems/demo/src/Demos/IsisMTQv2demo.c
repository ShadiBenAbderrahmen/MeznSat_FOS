...
/***
 * Initializes the I2C interface driver and the IMTQv2 subsystem driver
...
static Boolean _initMain(void)
{
	...
	if(rv != E_NO_SS_ERR && rv != E_IS_INITIALIZED)				// rinse-repeat
	{
		...
		// hanging the code
		TRACE_ERROR("\n\r IsisMTQv2demo IsisMTQv2_initialize() failed; err=%d! Exiting ... \n\r", rv);
		return FALSE;
	}
...
Boolean IsisMTQv2test()
{

	if(_initMain())											// initialize of I2C and IMTQv2 subsystem drivers succeeded?
	{
		_mainDemo();										// show the main IMTQv2 demo interface and wait for user input
	}
