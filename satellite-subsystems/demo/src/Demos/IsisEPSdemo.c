...
static Boolean _initMain(void)
{
	...
	if(rv != E_NO_SS_ERR && rv != E_IS_INITIALIZED)
	{
		...
		// hanging the code
		TRACE_ERROR("\n\r IsisEPSdemo IsisEPS_initialize() failed; err=%d! Exiting ... \n\r", rv);
		return FALSE;
	}
...
Boolean IsisEPStest()
{
	...
	if(_initMain())											// initialize of I2C and IEPS subsystem drivers succeeded?
	{
		_mainDemo();										// show the main IEPS demo interface and wait for user input
	}
