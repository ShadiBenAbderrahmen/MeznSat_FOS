...
static Boolean itc_sendDefClSignTest(void)
{
	...
	while(txCounter < 5 && timeoutCounter < 5)
	{
		...
		IsisTrxuv_itcSendAX25DefClSign(0, testBuffer1, 10, &avalFrames);
...
static Boolean itc_sendMsgCWTest(void)
{
	...
	while(txCounter < TX_UPBOUND && timeoutCounter < TIMEOUT_UPBOUND)
	{
		...
		IsisTrxuv_itcSendCwMessage(0, cwBuffer, 20, &avalFrames);
...
static Boolean itc_toggleIdleStateTest(void)
{
	...
	if(toggle_flag)
	{
		IsisTrxuv_itcSetIdlestate(0, trxuv_idle_state_off);
		toggle_flag = FALSE;
	}
	else
	{
		IsisTrxuv_itcSetIdlestate(0, trxuv_idle_state_on);
		toggle_flag = TRUE;
	}
...
{
	IsisTrxuv_itcSetAx25Bitrate(0, trxuv_bitrate_9600);
...
{
	IsisTrxuv_itcSetAx25Bitrate(0, trxuv_bitrate_1200);
...
static Boolean imc_getFrameCountTest(void)
{
	...
	while(timeoutCounter < 4*TIMEOUT_UPBOUND)
	{
		...
		IsisTrxuv_imcGetFrameCount(0, &RxCounter);
...
static Boolean imc_getFrameCmdTest(void)
{
	...
	while(RxCounter > 0)
	{
		...
		if(RxCounter != 0)
		{
			IsisTrxuv_imcGetCommandFrame(0, &rxFrameCmd);
...
static Boolean imc_getAllTelemTest(void)
{
	...
	IsisTrxuv_imcGetTelemetryAll(0, &telemetry);
...
static Boolean selectAndExecuteTRXUVDemoTest(void)
{
	...
	printf("\t 8) Get all telemetry \n\r");
	printf("\t 9) Reset TRXUV both microcontrollers \n\r");
...
static void initmain(void)
{
	...
	IsisTrxuv_initialize(myTRXUVAddress, myTRXUVBuffers, myTRXUVBitrates, 1);
}
