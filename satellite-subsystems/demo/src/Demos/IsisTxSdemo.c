...
static Boolean halfMaxBitrateTxsTest(void)
{
	...
	IsisTxsSetAx25Bitrate(0, txs_bitrate_half);
...
static Boolean MaxBitrateTxsTest(void)
{
	...
	IsisTxsSetAx25Bitrate(0, txs_bitrate_full);
...
static Boolean level4_5dBTxsTest(void)
{
	...
	IsisTxsSetModulatorAttenuator(0, txs_4_5dB_attenuation);
...
static Boolean level13_5dBTxsTest(void)
{
	...
	IsisTxsSetModulatorAttenuator(0, txs_13_5dB_attenuation);
...
static Boolean toggleTxsStateTest(void)
{
	...
	if(toggle_flag)
	{
		IsisTxsSetState(0, txs_state_off);
		toggle_flag = FALSE;
	}
	else
	{
		IsisTxsSetState(0, txs_state_on);
		toggle_flag = TRUE;
	}
...
static Boolean addDefaultCallsignFrames(void)
{
	...
	while(txCounter < 5 && timeoutCounter < 5)
	{
		...
		IsisTxsSendAX25DefClSign(0, testBuffer1, 10, &txsbuffinfo);
...
static Boolean getAllTelemTxSTest(void)
{
	...
	printf("\r\nGet all Telemetry \r\n\r\n");
	IsisTxsGetHousekeepingData(0, &telemetry);
...
static void initmain(void)
{
    ...
    IsisTxsInitialize(&i2c_address, &maxframe_length, 1);
}

static void TxS_mainDemo(void)
{
	Boolean offerMoreTests = FALSE;

	while(1)
	{
		offerMoreTests = selectAndExecuteTxSDemoTest();
